...
func (r *DeviceAttrInfo) Insert() (code int, err error) {
	...
	st, err := db.Prepare("INSERT INTO dev_deviceattrinfo(device_sn, attr_code, attr_name, attr_permission, attr_value_ctrl, is_control, attr_value_cur) VALUES(?, ?, ?, ?, ?, ?, ?, ?)")
	if err != nil {
	...
	if _, err := st.Exec(r.Device_sn, r.Attr_code, r.Attr_name, r.Attr_permission, r.Attr_value_ctrl, r.Is_control, r.Attr_value_cur); err != nil {
...
func (r *DeviceAttrInfo) FindById(id int64) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, device_sn, attr_code, attr_name, attr_permission, attr_value_ctrl, is_control, attr_value_cur FROM dev_deviceattrinfo WHERE id = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
	...
	if err := row.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_name, &tmpAttr_permission, &tmpAttr_value_ctrl, &tmpIs_control, &tmpAttr_value_cur); err != nil {
...
func (r *DeviceAttrInfo) FindByAttrSN(device_sn string, attr_code int32) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, device_sn, attr_code, attr_name, attr_permission, attr_value_ctrl, is_control, attr_value_cur FROM dev_deviceattrinfo WHERE attr_code = ? and device_sn = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
...
func GetAllDeviceAttrInfos(queryVal map[string]string, queryOp map[string]string,
	...
	sqlStr := "SELECT id, device_sn, attr_code, attr_name, attr_permission, attr_value_ctrl, is_control, attr_value_cur FROM dev_deviceattrinfo"
	if len(queryVal) > 0 {
		sqlStr += " WHERE "
		first := true
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_name, &tmpAttr_permission, &tmpAttr_value_ctrl, &tmpIs_control, &tmpAttr_value_cur); err != nil {
			return nil, err
		}
