...
func (r *DeviceCtrl) Insert() (code int, err error) {
	...
	st, err := db.Prepare("INSERT INTO dev_devicectrl(device_sn, attr_code, attr_value, source, ctrltime) VALUES(?, ?, ?, ?, ?, ?)")
	if err != nil {
	...
	if _, err := st.Exec(r.Device_sn, r.Attr_code, r.Attr_value, r.Source, r.CtrlTime); err != nil {
...
func (r *DeviceCtrl) FindById(id int64) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, device_sn, attr_code, attr_value, source, ctrltime FROM dev_devicectrl WHERE id = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
	...
	if err := row.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_value, &tmpSource,
		&tmpCtrlTime); err != nil {
...
func GetAllDeviceCtrls(queryVal map[string]string, queryOp map[string]string,
	...
	sqlStr := "SELECT id, device_sn, attr_code, attr_value, source FROM dev_devicectrl"
	if len(queryVal) > 0 {
		sqlStr += " WHERE "
		first := true
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_value, &tmpSource, 
		&tmpCtrlTime); err != nil {
			return nil, err
		}
