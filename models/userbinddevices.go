...
func (r *UserBindDevice) Insert() (code int, err error) {
	...
	st, err := db.Prepare("INSERT INTO dev_user_bind_device(username, device_sn) VALUES(?, ?, ?)")
	if err != nil {
	...
	if _, err := st.Exec(r.Username, r.Device_sn); err != nil {
...
func (r *UserBindDevice) FindById(id int64) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, username, device_sn FROM dev_user_bind_device WHERE id = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
	...
	if err := row.Scan(&tmpId, &tmpUsername, &tmpDevice_sn); err != nil {
...
func (r *UserBindDevice) FindByUsernameDeviceSN(username string, device_sn string) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, username, device_sn FROM dev_user_bind_device WHERE username = ? and device_sn = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
...
func GetAllUserBindDevices(queryVal map[string]string, queryOp map[string]string,
	...
	sqlStr := "SELECT id, username, device_sn FROM dev_user_bind_device"
	if len(queryVal) > 0 {
		sqlStr += " WHERE "
		first := true
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tmpId, &tmpUsername, &tmpDevice_sn); err != nil {
			return nil, err
		}
