...
func (r *DeviceReport) Insert() (code int, err error) {
	...
	st, err := db.Prepare("INSERT INTO dev_devicereport(device_sn, attr_code, attr_value, reporttime) VALUES(?, ?, ?, ?, ?)")
	if err != nil {
	...
	if _, err := st.Exec(r.Device_sn, r.Attr_code, r.Attr_value, r.ReportTime); err != nil {
...
func (r *DeviceReport) FindById(id int64) (code int, err error) {
	...
	st, err := db.Prepare("SELECT id, device_sn, attr_code, attr_value, reporttime FROM dev_devicereport WHERE id = ?")
	if err != nil {
		return ErrDatabase, err
	}
	defer st.Close()
	...
	if err := row.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_value, 
...
func GetAllDeviceReports(queryVal map[string]string, queryOp map[string]string,
	...
	sqlStr := "SELECT id, device_sn, attr_code, attr_value, reporttime FROM dev_devicereport"
	if len(queryVal) > 0 {
		sqlStr += " WHERE "
		first := true
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tmpId, &tmpDevice_sn, &tmpAttr_code, &tmpAttr_value,
		&tmpReportTime); err != nil {
			return nil, err
		}
